...
// Gui Ma tran A tu Core 0 den tat ca cac Core
  ...
  //   }
  //   for (i=1;i<NP;i++)
  //     MPI_Send(A+i*mc*n,mc*n,MPI_INT,i,i,
  //              MPI_COMM_WORLD);
  // } else
  // {
  //   MPI_Recv(Ac,mc*n,MPI_INT,0,rank,
...
// Gui Ma tran B tu Core 0 den tat ca cac Core
  ...
  //   }
  //   for (i=1;i<NP;i++)
  //   MPI_Send(B,n*p,MPI_INT,i,i,
  //            MPI_COMM_WORLD);
  // } else
  // {
  //   MPI_Recv(Bc,n*p,MPI_INT,0,rank,
  //            MPI_COMM_WORLD,&status);    
  // }
...
// Gui Ma tran C tu tat ca cac Core ve Core 0
  // if (rank!=0){
  //   MPI_Send(Cc,mc*p,MPI_INT,0,rank,
  //            MPI_COMM_WORLD);
  // } else{
  //   for ( i = 0 ; i < mc ; i++ )
  //     for ( j = 0 ; j < p ; j++ )
  //      *(C+i*p+j) = *(Cc+i*p+j);
  //   for (i = 1; i < NP; i++)
  //     MPI_Recv(C + i*mc*p,mc*n,MPI_INT,i,i,
  //              MPI_COMM_WORLD,&status);
  // }
